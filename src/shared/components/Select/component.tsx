...
export const Select = ({ label, options, selected, onChange, className, labelClassName }: SelectProps) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className={cn('relative w-[300px]', className)}>
      {label && <label className={cn('block text-xl text-base-grey-07 mb-2', labelClassName)}>{label}</label>}
      <Button
        ...
        <ul className="absolute w-full bg-base-grey-01 border rounded-lg shadow-md mt-0.5 z-10">
          {options.map(option => (
            <li
              ...
              className="px-3 py-1 text-base font-medium text-base-grey-09 hover:bg-base-grey-03 cursor-pointer"
            >
              {option.label}
            </li>
          ))}
